...
def main():
    ...
    for N in iter([12, 24, 48, 96]):
        study_name_1 = f'JC_unet_{cond}_{datatype}_N_{N}'
        storage_name_1 = "sqlite:///data/tune/{}.db".format(study_name_1)
        study_1 = optuna.create_study(study_name=study_name_1, storage=storage_name_1, direction='minimize', sampler=optuna.samplers.TPESampler(), pruner=optuna.pruners.HyperbandPruner(), load_if_exists=True)
        param_1 = study_1.best_trial.params
        n_units = lib.cal_units_dae(param_1['n_layers'], param_1['nodes'], param_1['multiple'])
        path = f'data/unet/{cond}/{datatype}/N_{N}_model.pth'
        if param_1['n_layers'] == 3:
            unet = lib.DenoisingUNet3(N, n_units, device, param_1['act'], param_1['alpha'])
        elif param_1['n_layers'] == 5:
            unet = lib.DenoisingUNet5(N, n_units, device, param_1['act'], param_1['alpha'])
        elif param_1['n_layers'] == 7:
            unet = lib.DenoisingUNet7(N, n_units, device, param_1['act'], param_1['alpha'])
        unet.load_state_dict(torch.load(path, map_location=device))

        study_name_2 = f'JC_dnn_{cond}_{datatype}_N_{N}'
        storage_name_2 = "sqlite:///data/tune/{}.db".format(study_name_2)
        study_2 = optuna.create_study(study_name=study_name_2, storage=storage_name_2, direction='minimize', sampler=optuna.samplers.TPESampler(), pruner=optuna.pruners.HyperbandPruner(), load_if_exists=True)
        param_2 = study_2.best_trial.params
        path = f'data/dnn/{cond}/{datatype}/N_{N}_model.pth'
        dnn = lib.DNN(N-1, param_2['n_units'], param_2['n_layers'], device, path, param_2['act'], load_param=True)

        os.makedirs(f'data/comp_vanilla/{cond}/{datatype}', exist_ok=True)
        model = lib.UNet_DNN(unet, dnn)
        for eps in iter([0, 0.2, 0.4, 0.6, 0.8]):
            ...
            with open(f'data/comp_vanilla/{cond}/{datatype}/N_{N}_test_results_eps_{eps}.txt', 'w') as f:
                f.write(f'RMSE:\n')
...
total loss: {rmse_error["running_loss"]} (var: {rmse_error_var["running_loss"]}),\
    e_loss: {rmse_error["e_loss"]} (var: {rmse_error_var["e_loss"]})\n')
                print('RMSE:')
